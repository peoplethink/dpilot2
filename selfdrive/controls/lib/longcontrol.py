...
class LongControl:
  ...
  def update(self, active, CS, long_plan, accel_limits, t_since_plan):
    ...
    if len(speeds) == CONTROL_N:
      ...
      v_target = interp(ntune_common_get('longitudinalActuatorDelay') + t_since_plan, T_IDXS[:CONTROL_N], speeds)
      a_target = 2 * (v_target - v_target_now) / ntune_common_get('longitudinalActuatorDelay') - a_target_now

      v_target_1sec = interp(ntune_common_get('longitudinalActuatorDelay')+ t_since_plan + 1.0, T_IDXS[:CONTROL_N], speeds)
    else:
      v_target = 0.0
      v_target_now = 0.0
      v_target_1sec = 0.0
