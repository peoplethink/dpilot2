...
static void update_model(UIState *s, const cereal::ModelDataV2::Reader &model) {
  ...
  for (int i = 0; i < std::size(scene.lane_line_vertices); i++) {
    ...
    update_line_data(s, lane_lines[i], 0.045 * scene.lane_line_probs[i], 0, 0, &scene.lane_line_vertices[i], max_idx);
  }
  ...
  for (int i = 0; i < std::size(scene.road_edge_vertices); i++) {
    ...
    update_line_data(s, road_edges[i], 0.035, 0, 0, &scene.road_edge_vertices[i], max_idx);
  }
  ...
  update_line_data(s, model_position, 0.5, 1.22, 1.22, &scene.track_vertices, max_idx, false);
}
