...
class CarInterface(CarInterfaceBase):
  ...
  def get_pid_accel_limits(CP, current_speed, cruise_speed):
    ...
    gas_max_v = [1.55, 1.34, 1.13, 0.53, 0.38, 0.15, 0.1]
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, disable_radar):
    ...
    ret.longitudinalTuning.kpV = [1.3, 1.23, 0.92, 0.58, 0.35]
    ret.longitudinalTuning.kiBP = [0., 130. * CV.KPH_TO_MS]
    ret.longitudinalTuning.kiV = [0.08, 0.05]

    ret.stoppingControl = True
    ret.startingState = False
    ret.vEgoStarting = 0.2 #ntune_scc_get('vEgoStarting')
    ret.vEgoStopping = 0.2 #ntune_scc_get('vEgoStopping')
